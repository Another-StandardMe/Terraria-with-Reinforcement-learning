...
class Args:
    ...
    K_epochs = 3
    entropy_coef = 0.005
    max_train_steps = 1.2e6 # 500epoch:4e5, 1800epoch:15e5
    batch_size = 4
...
class Worker(mp.Process):
    ...
    def run(self):
        ...
        for epoch in range(self.args.max_epochs):
            ...
            print(f"✅ Worker {self.worker_id} 结束 Epoch {epoch + 1} | ep_reward: {ep_reward}")
            env.close()
            # 发送本轮结束信号
...
class Learner(mp.Process):
    ...
    def run(self):
                ...
                if isinstance(data, tuple):
                    ...
                    if data[0] == "TERMINATE":
                        ...
                        with open(f"epoch_record_{self.args.date}.txt", "a") as f:
                            f.write(
                                f"epoch {epoch_num}: step:{ep_length}, avg_policy_loss:{ep_avg_losses:.4f}, avg_q_value:{ep_avg_qs:.4f}, reward:{ep_reward:.4f}, kill: {kill}\n")
        ...
        save_path = "checkpoints/Terraria_final_model_1.pth"
        os.makedirs("checkpoints", exist_ok=True)
        torch.save({
