...
class LiteCNNTransformer(nn.Module):
    ...
    def forward(self, images):
        """ 处理 `[8, 3, 224, 224]` 并输出策略动作 """
        if images.ndim == 4:  # 处理单帧情况
            images = images.unsqueeze(0)  # 变为 [1, 8, 3, 224, 224]

        B, N, C, H, W = images.shape  # [1, 8, 3, 224, 224]

        # **CNN 提取每一帧的特征**
        cnn_features = self.extract_cnn_feature(images.view(N, C, H, W))  # [8, img_feature_dim]
        cnn_features = cnn_features.unsqueeze(0)  # [1, 8, img_feature_dim]

        # **Transformer 处理时序信息**
        state_emb = self.state_proj(cnn_features)
        state_emb = self.positional_encoding(state_emb)
        transformer_out = self.transformer(state_emb)[:, -1, :]  # 只取最后一帧输出 [1, transformer_dim]

        # **策略头**
        hidden = self.hidden_fc(transformer_out)
        move_logits = self.move_output(hidden)  # [1, 3]
        jump_logits = self.jump_output(hidden)  # [1, 2]
        down_logits = self.down_output(hidden)  # [1, 2]

        return move_logits.squeeze(0), jump_logits.squeeze(0), down_logits.squeeze(0)
