    ...
    def run(self):
        ...
        for epoch in range(self.args.max_epochs):
            ...
            while not done:
                ...
                print(np.array(action).shape, f"action: {action}")  # ✅ 转换为 numpy 数组再打印
                ...
                try:
                    self.data_queue.put_nowait((
                        obs.cpu(),  # [8, 3, 224, 224]
                        torch.tensor(action, dtype=torch.long),  # [3]
                        torch.tensor(log_prob, dtype=torch.float),  # 标量
                        torch.tensor(reward, dtype=torch.float),  # 标量
                        next_obs.cpu(),  # [8, 3, 224, 224]
                        torch.tensor(float(done), dtype=torch.float)  # 0.0 或 1.0
                    ))
                    self.epoch_sync_event.set()  # ✅ 立即通知 Learner 训练
                except:
...
class Learner(mp.Process):
    ...
    def run(self):
        ...
        while not self.exit_event.is_set():
            try:
                ...
                if data is None:
                    print("⚠️ Learner 收到终止信号，退出")
                    break  # ✅ 退出循环

                batch_samples.append(data)
                self.total_samples += 1  # ✅ 记录训练数据量

                # **每 16 个样本训练一次**
                if len(batch_samples) >= 16:
                    self.model.update(batch_samples)
                    batch_samples = []  # ✅ 清空 batch，准备下一个

                    # **每训练 5600 个数据（700 batch）保存一次 `checkpoint.pth`**
    ...
    #                 save_path = f"checkpoints/Terraria_model_{self.iteration}.pth"
    #                 os.makedirs("checkpoints", exist_ok=True)
    #                 torch.save(self.model.actor.state_dict(), save_path)
